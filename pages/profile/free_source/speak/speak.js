// Go back function
function goBack() {
    window.location.href = '../free_source.html';
}

const speakSourceData = [
    {
        id: "1",
        title: "a - âm /æ/",
        description: "Học cách phát âm và nói tiếng Anh cơ bản. Hãy luyện tập cùng các bài tập tương tác.",
        type: "free",
        image: "https://img.freepik.com/free-vector/you-speak-english-lettering-background_23-2147867240.jpg?semt=ais_hybrid&w=740&q=80",
        lessons: [
            {
                id: 1,
                title: "Video 1",
                videos: [
                    { id: 1, title: "Video 1", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video1.mp4" }
                ]
            },
            {
                id: 2,
                title: "Video 2",
                videos: [
                    { id: 2, title: "Video 2", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video2.mp4" }
                ]
            },
            {
                id: 3,
                title: "Video 3",
                videos: [
                    { id: 3, title: "Video 3", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video3.mp4" }
                ]
            }
        ]
    },
    {
        id: "2",
        title: "b - âm /b/",
        description: "Học cách phát âm và nói tiếng Anh cơ bản. Hãy luyện tập cùng các bài tập tương tác.",
        type: "free",
        image: "https://img.freepik.com/free-vector/you-speak-english-lettering-background_23-2147867240.jpg?semt=ais_hybrid&w=740&q=80",
        lessons: [
            {
                id: 1,
                title: "Video 1",
                videos: [
                    { id: 1, title: "Video 1", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video1.mp4" }
                ]
            },
            {
                id: 2,
                title: "Video 2",
                videos: [
                    { id: 2, title: "Video 2", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video2.mp4" }
                ]
            },
            {
                id: 3,
                title: "Video 3",
                videos: [
                    { id: 3, title: "Video 3", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video3.mp4" }
                ]
            }
        ]
    },
    {
        id: "3",
        title: "c - âm /k/",
        description: "Học cách phát âm và nói tiếng Anh cơ bản. Hãy luyện tập cùng các bài tập tương tác.",
        type: "free",
        image: "https://img.freepik.com/free-vector/you-speak-english-lettering-background_23-2147867240.jpg?semt=ais_hybrid&w=740&q=80",
        lessons: [
            {
                id: 1,
                title: "Video 1",
                videos: [
                    { id: 1, title: "Video 1", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video1.mp4" }
                ]
            },
            {
                id: 2,
                title: "Video 2",
                videos: [
                    { id: 2, title: "Video 2", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video2.mp4" }
                ]
            },
            {
                id: 3,
                title: "Video 3",
                videos: [
                    { id: 3, title: "Video 3", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video3.mp4" }
                ]
            }
        ]
    },
    {
        id: "4",
        title: "d - âm /d/",
        description: "Học cách phát âm và nói tiếng Anh cơ bản. Hãy luyện tập cùng các bài tập tương tác.",
        type: "free",
        image: "https://img.freepik.com/free-vector/you-speak-english-lettering-background_23-2147867240.jpg?semt=ais_hybrid&w=740&q=80",
        lessons: [
            {
                id: 1,
                title: "Video 1",
                videos: [
                    { id: 1, title: "Video 1", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video1.mp4" }
                ]
            },
            {
                id: 2,
                title: "Video 2",
                videos: [
                    { id: 2, title: "Video 2", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video2.mp4" }
                ]
            },
            {
                id: 3,
                title: "Video 3",
                videos: [
                    { id: 3, title: "Video 3", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video3.mp4" }
                ]
            }
        ]
    },
    {
        id: "5",
        title: "e - âm /i:/",
        description: "Học cách phát âm và nói tiếng Anh cơ bản. Hãy luyện tập cùng các bài tập tương tác.",
        type: "pay",
        image: "https://img.freepik.com/free-vector/you-speak-english-lettering-background_23-2147867240.jpg?semt=ais_hybrid&w=740&q=80",
        lessons: [
            {
                id: 1,
                title: "Video 1",
                videos: [
                    { id: 1, title: "Video 1", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video1.mp4" }
                ]
            },
            {
                id: 2,
                title: "Video 2",
                videos: [
                    { id: 2, title: "Video 2", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video2.mp4" }
                ]
            },
            {
                id: 3,
                title: "Video 3",
                videos: [
                    { id: 3, title: "Video 3", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video3.mp4" }
                ]
            }
        ]
    },
    {
        id: "6",
        title: "f - âm /f/",
        description: "Học cách phát âm và nói tiếng Anh cơ bản. Hãy luyện tập cùng các bài tập tương tác.",
        type: "pay",
        image: "https://img.freepik.com/free-vector/you-speak-english-lettering-background_23-2147867240.jpg?semt=ais_hybrid&w=740&q=80",
        lessons: [
            {
                id: 1,
                title: "Video 1",
                videos: [
                    { id: 1, title: "Video 1", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video1.mp4" }
                ]
            },
            {
                id: 2,
                title: "Video 2",
                videos: [
                    { id: 2, title: "Video 2", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video2.mp4" }
                ]
            },
            {
                id: 3,
                title: "Video 3",
                videos: [
                    { id: 3, title: "Video 3", url: "https://appv2.tienganh123.com/data/videos/child_pronun/child_pronun/unit1/unit1_video3.mp4" }
                ]
            }
        ]
    }
];

// Render source items
function renderSourceItems() {
    const container = document.getElementById('sourceContainer');
    
    speakSourceData.forEach(source => {
        const sourceCard = document.createElement('div');
        sourceCard.className = 'source-item';
        sourceCard.innerHTML = `
            <div class="source-image-wrapper">
                <img src="${source.image}" alt="${source.title}" class="source-image">
                <span class="source-type ${source.type}">${source.type === 'free' ? 'Miễn phí' : 'Trả phí'}</span>
            </div>
            <h3 class="source-title" data-id="${source.id}">${source.title}</h3>
            <p class="source-description">${source.description}</p>
        `;
        
        // Add click event to title
        const titleElement = sourceCard.querySelector('.source-title');
        titleElement.addEventListener('click', () => {
            window.location.href = `./detail/source_detail.html?id=${source.id}`;
        });
        
        container.appendChild(sourceCard);
    });
}

// Initialize on page load
document.addEventListener('DOMContentLoaded', () => {
    renderSourceItems();
});

